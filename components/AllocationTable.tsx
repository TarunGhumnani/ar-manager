  ...
  return (
    <div>
            ...
            <tr key={r.id} className="border-t border-slate-100">
              ...
              <td className="px-2 py-1.5 text-right">
                <input
                  ...
                  inputMode="decimal"
                  className={`w-32 rounded border px-2 py-1 text-right ${Number.isNaN(amounts[i]) || amounts[i] > r.outstanding ? 'border-red-500' : 'border-slate-300'}`}
                />
                <button type="button" className="ml-1 text-xs text-blue-700 underline"
      ...
      <div className="mt-3 flex flex-wrap gap-6 text-sm">
        ...
        <span className={left < 0 ? 'font-semibold text-red-700' : ''}>
          Remains unapplied: <b>{fmt(left)}</b>{left < 0 && ' (over-allocated)'}
        </span>
        <button type="button" className="text-blue-700 underline" onClick={() => setVals(Object.fromEntries(rows.map((r) => [r.id, ''])))}>
          Clear all
        </button>
      </div>
      {bad && <p className="mt-2 text-sm text-red-700">An amount is not a number or is more than the invoice&apos;s outstanding.</p>}
    </div>
  );
}
